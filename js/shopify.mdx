---
title: Shopify
source_url: https://fly.io/docs/js/shopify/
---
...
The value of `SHOPIFY_API_SECRET` will be set as a [fly secret](/apps/secrets).  The remainder will be placed into your `fly.toml`:
...
* Setting `min_machines_running` to 1 in the [http service section](/reference/configuration#the-http_service-section) of your `fly.toml` can make sure that there always is a started machine to process requests.
* setting `auto_stop_machines` to `off` in the [http service section](/reference/configuration#the-http_service-section) will prevent your Machines from being stopped all together.

You can also [Scale Machine CPU and RAM](/launch/scale-machine), and  [Scale the number of Machines](/launch/scale-count).
...
Shopify's template starts you off with SQLite using Prisma.  Of particular note:
  ...
  * For PostgreSQL applications without any volumes, [rolling deploys](/launch/deploy#deployment-strategy) are the default, avoiding any downtime.
