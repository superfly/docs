...
og:description: NextJS is the React Framework for the Web.
source_url: https://fly.io/docs/js/frameworks/nextjs/
---
...
In this guide we’ll learn how to deploy a [Next.js](https://nextjs.org/) application on Fly.io.
...
In the process we've generated a [`fly.toml`](/reference/configuration) and `Dockerfile` in the root of the project. Change them if you need to customize your deployment.
****
...
It is possible to mount multiple secrets, and you can read more about [mounting secrets](/apps/build-secrets#mounting-secrets) for further details.
...
* Your deployment Machines will need enough memory to run a build.  See:
  [`fly scale memory`](/flyctl/cmd/fly_scale_memory) and
  [`swap_size_mb`](/reference/configuration#swap_size_mb-option) for two options.
* You may need to adjust the [grace_period](/reference/configuration#http_service-checks) for any HTTP service checks.
...
Two links that may be of help:
  ...
  * [fly scale memory](/flyctl/cmd/fly_scale_memory)
  * [swap_size_mb](/reference/configuration#swap_size_mb-option)
