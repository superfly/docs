---
title: Sqlite
source_url: https://fly.io/docs/js/prisma/sqlite/
---
...
Sqlite publishes a page of [appropriate uses of SQLite](https://www.sqlite.org/whentouse.html). There are few downsides to be aware of:
  ...
  * You are effectively limited to a single machine.  Machines can scale quite large these days, but requests from around the world will need to make the transit and back to this machine.  [LiteFS](/litefs) can provide read replicas.
...
In order to survive restarts and deploys, your database will be placed on a [volume](/volumes/overview).  This is controlled by the [mounts section](/reference/configuration#the-mounts-section) in your `fly.toml`:
