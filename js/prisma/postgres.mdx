---
title: PostgreSQL
source_url: https://fly.io/docs/js/prisma/postgres/
---
...
`fly launch` will create a [PostgreSQL database](/postgres) suitable for development.
For production, see our [list of recommended providers](/postgres/getting-started/what-you-should-know#recommended-external-providers).
...
controlled by your [`fly.toml`](/reference/configuration#run-one-off-commands-before-releasing-a-deployment):
...
Import the resulting database.  For Fly Postgres use [`fly postgres import`](/flyctl/cmd/fly_postgres_import).

Set the `DATABASE_URL` secret using [`fly secrets set`](/flyctl/cmd/fly_secrets_set).

Run `fly deploy`.

Once you are up and running, you can delete your volume using [`fly volumes destroy`](/flyctl/cmd/fly_volumes_destroy).
