---
title: Databases
og:description: Connecting your application to a database.
source_url: https://fly.io/docs/js/the-basics/database/
---

Most web servers store data in a database, and there are many databases to chose from.
Examples include running [PostgreSQL](/unmanaged-postgres) on fly.io's infrastructure,
...
[`fly secrets set`](/apps/secrets#set-secrets).
...
[Fly.io private network](/networking/private-networking), you will _not_
...
[`release_command`](/reference/configuration#run-one-off-commands-before-releasing-a-deployment) in your `fly.toml` file.
