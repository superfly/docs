...
source_url: https://fly.io/docs/networking/udp-and-tcp/
---
...
You'll need a [dedicated IPv4 address](/networking/services#dedicated-ipv4) for your app to accept UDP packets. We don't support UDP over public IPv6. Every Fly.io app gets a public IPv6 address and a shared IPv4 address that will work for the TCP side of your app.

<Info>
**Note:** UDP does work over [Fly.io IPv6 private networking](/networking/private-networking).
</Info>
...
### Let's See Some Code {#lets-see-some-code}
...
This program, [with a simple Dockerfile](https://github.com/fly-apps/udp-echo-/blob/main/Dockerfile) and the `fly.toml` configuration above, should just work on Fly.io. [Boot it up](/flyctl/cmd/fly_deploy) and throw some UDP packets at it with netcat:
