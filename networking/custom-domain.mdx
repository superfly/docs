...
source_url: https://fly.io/docs/networking/custom-domain/
---

  <img src="/images/docs-ui.webp" alt="" />
...
We support the top-level domains on the [IANA list](https://data.iana.org/TLD/tlds-alpha-by-domain.txt). Note that we only periodically update top-level domain support and there might be a delay before we add support for new top-level domains.
...
In these cases, you can bypass ACME certificate generation entirely by importing a [Cloudflare Origin Certificate](/networking/understanding-cloudflare#using-a-cloudflare-origin-certificate) and verifying ownership with the same `_fly-ownership` TXT record.

## Related reading

- [Networking overview](/networking) Broader look at how Fly.io handles routing, DNS, IPs, and certificates.
- [Certificates API reference](/machines/api/certificates-resource) Use the Machines API to automate cert provisioning and domain verification.
- [TLS termination by Fly Proxy](/security/tls-termination) — How Fly.io handles HTTPS for your apps and manages TLS certificates automatically behind the scenes.
- [Understanding Cloudflare](/networking/understanding-cloudflare) What happens when you put Fly.io behind Cloudflare, and how to avoid common pitfalls.
