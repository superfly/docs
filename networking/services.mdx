...
source_url: https://fly.io/docs/networking/services/
---

  <img src="/images/public_network_services.png" alt="Illustration by Annie Ruygt of a ship carrying apps" />

Fly.io has public and private network services available. The public network services connect apps to the wider public internet, while the [private network services](/networking/private-networking) allow apps to communicate with other apps within the Fly.io private network.

## Anycast IP addresses

IPv6 addresses and shared IPv4 Anycast addresses are free. Dedicated IPv4 addresses are [billed](/about/pricing#anycast-ip-addresses) monthly.

<Info>
Anycast IP addresses described on this page are __not used__ for outbound connections made from within a Machine. See [Egress IP Addresses](/networking/egress-ips) for details.
</Info>
...
A new Fly App running a public service configured in [`fly.toml`](/reference/configuration) automatically gets a dedicated Anycast IPv6 address when it's first deployed.

<Info>
If you create apps with public services in their Machine config by using [`fly machine` commands](/machines/flyctl/fly-machine-run#define-a-fly-proxy-network-service) or the [Machines API](/machines/api), then you need to run `fly ips allocate-v6` to get a dedicated IPv6 address.
</Info>

### Shared IPv4

Along with the dedicated IPv6 address, apps are automatically given a shared Anycast IPv4 address on the first deployment when they're configured to handle one or both of the following protocols and ports in [`fly.toml`](/reference/configuration):

* HTTP on port 80, or
* TLS & HTTP on port 443

This includes apps configured with the simplified [`[http_service]`](/reference/configuration#the-http_service-section) section, which handles only these protocols and ports by default.
...
<Note>
If you create apps with public services in their Machine config using [`fly machine` commands](/machines/flyctl/fly-machine-run#define-a-fly-proxy-network-service) or the [Machines API](/machines/api), then you need to run `fly ips allocate-v4 --shared` to get a shared IPv4 address.
</Note>

### Dedicated IPv4

Dedicated IPv4 addresses are [billed](/about/pricing#anycast-ip-addresses) monthly.
...
2. If you don't use a CNAME to `.fly.dev`, [add the shared IPv4 as an A record](/networking/custom-domain#set-the-a-record).
...
4. If you are using a custom domain and don't already have an SSL certificate for this app, [create one](/networking/custom-domain#add-your-domain-to-your-app). The cert is required for routing to your app via the custom domain, even for HTTP connections.
...
Set the `handlers` config setting in the [`services.ports` section](/reference/configuration#services-ports) of your `fly.toml` file.
...
You can also configure [TLS options](/reference/configuration#http_service-tls_options) for the `[http_service]` section, which has a TLS (and HTTP) handler by default on port 443.
...
### Find your Machine's outbound IP {#find-your-machines-outbound-ip}

The Machine's outbound IPv6 address is available in the `FLY_PUBLIC_IP` [environment variable](/machines/runtime-environment). Use `fly ssh console -s` to get an interactive shell on the Machine of your choice.
...
All requests to Fly.io apps are routed to our edge servers, where we use our memory-safe Rust proxy to direct traffic. The proxy listens on all ports and all Anycast addresses, so ports appear to be 'open' from the wider internet. However, nothing on your app is exposed unless you expose it via [services](/networking/app-services). You’re locked down by default.

## Related reading

- [Connect to an App Service](/networking/app-services) How to expose your Fly App’s processes publicly or privately through services.
- [Dynamic Request Routing with fly‑replay](/networking/dynamic-request-routing) How to route requests between regions, apps, or machines using Fly‑specific headers.
- [Connect Your Private Network with WireGuard](/blueprints/connect-private-network-wireguard) Set up secure access between Fly.io and your private infrastructure using WireGuard tunnels.
- [Private Networking](/networking/private-networking) How services communicate securely over Fly.io’s private network without going through the public internet.
