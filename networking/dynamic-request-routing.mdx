...
source_url: https://fly.io/docs/networking/dynamic-request-routing/
---
...
When your app responds to a request with a `fly-replay` response, [Fly Proxy](/reference/fly-proxy) will automatically replay the original request according to your specified routing rules. This enables advanced patterns like:
...
<iframe width="100%" height="400" src="https://www.youtube.com/embed/riCh9Xeuf0s" title="YouTube video" frameborder="0" allowfullscreen></iframe>
...
|Field |Description |
|---|---|
|`region` | The region(s) to route the request to. Accepts comma-separated list of [region codes](/reference/regions) |
...
For more details on configuration options, see the [fly.toml configuration reference](/reference/configuration#http_service-http_options-replay_cache).
...
For session-based replay caching, set [allow_bypass](/reference/configuration#http_service-http_options-replay_cache-allow_bypass) to `true` in your `fly.toml` configuration for your `replay_cache` rule.
...
- Your app must use the [http handler](/networking/services#http-handler)
...
This header is useful for tracking request paths and implementing consistency patterns. See the [official Ruby client](https://github.com/superfly/fly-ruby/blob/main/lib/fly-ruby/regional_database.rb#L74-L76) for an example of using these fields to prevent read-your-write inconsistency.
...
If you replay with `prefer_instance` set, Fly Proxy will attempt to route to this Machine. This may not happen for a number of reasons, for example the Machine may not be found, or found but at its configured [hard_limit](/reference/load-balancing#load).
...
When using [global read replicas](/unmanaged-postgres/advanced-guides/high-availability-and-global-replication), use `fly-replay` to ensure write operations go to the primary region:

```
fly-replay: region=sjc
```

See our blueprint for [Multi-region databases and fly-replay](/blueprints/multi-region-fly-replay) for a complete implementation guide.
...
By default, cross-app routing works within the same organization and the same [private network](/networking/private-networking).
