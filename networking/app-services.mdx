...
source_url: https://fly.io/docs/networking/app-services/
---

  <img src="/images/docs-servers.webp" alt="" />
...
2. Over the WireGuard [IPv6 private network ("6PN")](/networking/private-networking) that the app belongs to&mdash;this can be useful for, e.g., providing private supporting services to your Fly Apps
...
Fly Proxy can load-balance requests for both public and private ([Flycast](/networking/flycast)) services among a Fly App's VMs. Routing to a service with Fly Proxy also enables other Fly Proxy features, such as [automatically starting and stopping Machines](/launch/autostop-autostart) in response to fluctuations in request traffic.
...
Our [private networking docs](/networking/private-networking#fly-io-internal-addresses) list the `.internal` names and what kind of information a DNS query returns for each.
...
[7] A Flycast IP [can be allocated](/networking/flycast) on a different private network from the app it points to, if both networks belong to the same org. This lets your apps reach your service in a different 6PN, if the service is configured to be available over Flycast.
