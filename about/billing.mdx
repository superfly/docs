...
source_url: https://fly.io/docs/about/billing/
---
...
New customers and all new organizations (including those created by previously existing customers) are billed monthly for resource usage. Refer to our resource [Pricing page](/about/pricing) for details.
...
For example, a Machine described in your dashboard as "shared-1x-cpu@1024MB" is the “shared-cpu-1x” Machine size preset, which comes with 256MB RAM, plus additional RAM (1024MB - 256MB = 768MB). For pricing and available CPU/RAM combinations, read about [Compute pricing](/about/pricing#compute).
...
Volume billing is pro-rated to the hour. For details, see [Volume pricing](/about/pricing#persistent-storage-volumes).
...
Volume snapshot billing is pro-rated to the hour and we subtract the free allowance first. For details, see [Volume Snapshots pricing](/about/pricing#volume-snapshots).
...
A **Linked Organization** can be created from the [new organization page](https://fly.io/organizations/new). When creating a new organization, you can choose an existing Organization to link it to.
...
- We recommend saving any invoices you may need for accounting purposes before linking an organization.

---

## Some things to consider

- Each organization has its own [private network](/networking/private-networking). Organizations are isolated from each other, which might
be advantageous from a security standpoint.
- Cross organization networking is still possible with [Flycast](/networking/flycast)
...
### If you don't have a credit card {#if-you-dont-have-a-credit-card}
...
On a legacy plan and wondering how it's billed? Read more about [Discontinued Plans](/about/discontinued-plans).
