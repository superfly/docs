...
source_url: https://fly.io/docs/mpg/client-configuration/
---
...
After [creating your MPG cluster](/postgres/create-and-connect) and attaching your app, these are the essentials:
...
See the [Phoenix guide](/postgres/guides-examples/phoenix-guide) for Elixir-specific migration setup.
...
All MPG clusters include PgBouncer for connection pooling. The pool mode you choose on the cluster side affects what your client can do. See [Cluster Configuration](/postgres/cluster-configuration) for how to change modes.
...
- **`LISTEN/NOTIFY`** doesn't work — use an alternative notifier (see the [Phoenix guide](/postgres/guides-examples/phoenix-guide) for Oban examples)
...
<AccordionGroup>
<Accordion title="Node.js — pg (node-postgres)">
```javascript
...
<Accordion title="Node.js — Prisma">
...
<Accordion title="Python — psycopg3 connection pool">
```python
import os
...
<Accordion title="Go — database/sql with pgx">
```go
import (
    "database/sql"
    "os"
...
<Accordion title="Ruby — ActiveRecord (Rails)">
```yaml
# config/database.yml
production:
  url: ENV["DATABASE_URL"]
...
<Accordion title="Elixir/Phoenix — Ecto">
```elixir
# config/runtime.exs
...
For comprehensive Phoenix setup including migrations, Oban configuration, and Ecto-specific troubleshooting, see the [Phoenix with Managed Postgres](/postgres/guides-examples/phoenix-guide) guide.
...
### `tcp recv (idle): closed` or `tcp recv (idle): timeout` {#tcp-recv-idle-closed-or-tcp-recv-idle-timeout}
...
### `ECONNRESET` or "connection reset by peer" {#econnreset-or-quot-connection-reset-by-peer-quot}
...
**Fix:** Ensure your app is configured for IPv6. For Elixir apps, see the [IPv6 settings guide](/elixir/getting-started#important-ipv6-settings). For other runtimes, verify that your DNS resolver supports AAAA records on the Fly private network.
