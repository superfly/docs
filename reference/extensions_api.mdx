...
source_url: https://fly.io/docs/reference/extensions_api/
---

Read our [Extensions Program overview](/about/extensions) before digging into this doc.

## Requirements

To implement the following, we need to provide you with:

* A shared secret for [signing provisioning requests](/reference/extensions_api#authentication-and-request-signing)
* A shared secret for [signing webhooks](/reference/extensions_api#webhook-authentication-and-request-signing)
...
Our [Flycast](/networking/flycast) internal load balancing feature allow you to route traffic from an IPv6 address on a customer network to one of your Fly.io applications.
...
By default, no traffic is routed to your app until it has services configured. See the [Machines API](/machines/api/machines-resource#create-a-machine) and [fly.toml docs](/reference/configuration#the-services-sections) for details.

Optionally, you can add the [proxy protocol handler](/networking/services#connection-handlers) to a service. We co-opt the [proxy protocol](https://github.com/haproxy/haproxy/blob/master/doc/proxy-protocol.txt) as a way to give your service knowledge of the IP address assigned on the customer network, at client connection time. This IP is useful for performing an additional security check beyond user/password credentials or tokens.
...
Webhook requests should be signed the same way as we [sign provisioning requests](/reference/extensions_api#authentication-and-request-signing) but with a different shared secret.
