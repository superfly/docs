...
source_url: https://fly.io/docs/reference/fly-launch/
---

Fly Launch is a bundle of features that take a lot of the work out of deploying and managing your Fly App. Two commands that you'll use often are [`fly launch`](/flyctl/cmd/fly_launch) and [`fly deploy`](/launch/deploy). 
...
Reference: [Builders and Fly.io](/reference/builders)
...
Reference: [App configuration (fly.toml)](/reference/configuration)

## Platform resource provisioning

Before deployment, you might want to create and configure a [Fly Volume](/volumes) or [Postgres database](/unmanaged-postgres), or [set app secrets](/apps/secrets).
...
For more information about ways to customize your launch, refer to [Custom launch](/launch/create#custom-launch) and check out all the [options](/flyctl/cmd/fly_launch) available for use with `fly launch`.
...
* Next, I check the port for services; this port is the `internal_port` in the [`[[http_service]]` section](/reference/configuration#the-services-sections) of the `fly.toml` file. My Flask app listens on port 4999, not the more common port 8080 (the `fly.toml` default), so my Dockerfile contains the line:
...
Because I had an HTTP service configured, and no [public IP addresses](/networking/services), Fly Launch [provisioned the IPs on deployment](/launch/deploy#ip-addresses).
...
I haven't explicitly configured [process groups](/launch/processes), so my app gets two Machines assigned to the default `app` process. With `fly status`, I can see the Machines:
...
<Info>
**Note:** Both Machines are in a stopped state, because they were idle for a few minutes and we enable the [autostop/autostart feature](/launch/autostop-autostart) by default.
</Info>

I can [scale out](/launch/scale-count) by adding Machines in other regions if I want to get close to users in more corners of the world.
