...
source_url: https://fly.io/docs/reference/content-encoding/
---
...
- [http_service](/reference/configuration#the-http_service-section) preconfigures the most common options for a service speaking HTTP. It will use the http handler implicitly for both ports 80 and 443 (and additionally, of course, the tls handler for port 443).
...
- A [service](/reference/configuration#the-services-sections) block explicitly using the http handler. This example serves http only (no https) on external port 80:
...
- [fly.toml reference](/reference/configuration)
- [How Fly.io handles HTTP](/networking/services)
- [Troubleshooting app performance](/apps/performance)
