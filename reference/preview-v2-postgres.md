...
## About V2 Postgres On Fly

V2 Postgres on Fly is a regular Fly.io app, with an automated creation process and some extensions to simplify management. It relies on building blocks available to all Fly apps, like `flyctl`, volumes, private networking, health checks, logs, metrics, and more. The source code is available on [GitHub](https://github.com/fly-apps/postgres-ha) to view and fork.
...
You can change VM resources for an individual machine with the `flyctl machine update` command:
...
The best way to add additional replicas at this time is through the `fly machine clone` command:
...
You can update a Postgres cluster that was provisioned with `flyctl postgres create`, to the latest [release](https://github.com/fly-apps/postgres-ha/releases) using [`flyctl pg update`](/docs/flyctl/postgres-update/).
