...
source_url: https://fly.io/docs/reference/load-balancing/
---

[Fly Proxy](/reference/fly-proxy) routes requests to individual Machines in your apps using a combination of concurrency settings specified on your app, current load, and closeness. This page describes the details of load balancing traffic to your apps on Fly.io.
...
Fly Proxy determines load using the [concurrency settings](/reference/configuration#services-concurrency) configured for an app and the current traffic relative to those settings.
...
We deploy 10 Machines in four regions: `ams` (Amsterdam), `nrt` (Tokyo), `sjc` (San Jose), and `sin` (Singapore), with three of those in `ams`.
