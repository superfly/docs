---
title: Run a Go App
source_url: https://fly.io/docs/languages-and-frameworks/golang/
---

  <img src="/images/golang.png" alt="Illustration by Annie Ruygt of a seated gopher dreaming" />
...
<Info>
Already have a Go Fly App? Try adding [Tigris](https://www.tigrisdata.com/docs/overview/) for file storage. Tigris is scalable, secure, global object storage and you can [integrate it quickly with your Go app](#add-tigris-global-storage-optional).
</Info>
...
Tigris provides CDN-like accessibility for your stored objects. Tigris has S3-API compatibility, automatically manages data replication and caching for you, and offers AuthN and AuthZ authentication mechanisms. Learn more about [Tigris Object Storage](https://www.tigrisdata.com/docs/overview/).
...
2. **Retrieve AWS SDK modules**: Tigris has an S3 compatible API service, thus, we can easily use [AWS SDK Go](https://aws.github.io/aws-sdk-go-v2/docs/getting-started/#install-the-aws-sdk-for-go-v2) modules to connect with the Tigris Bucket we've just created.
...
3. **Create a [Client function](https://github.com/fly-apps/go-example-tigris/blob/main/tigris/tigris.go#L12)** that will return an instance ready to connect with a Tigris bucket:
...
4. **Set up Tigris Credentials**. 
	...
	**In a local setup**, this should look something like:
	...
	```
	**For our Go Fly App**, these environment variables should have already been set as [secrets](/flyctl/cmd/fly_secrets) during the running of `fly storage create`:
	...
	But if that wasn't the case, we can still manually set necessary [secrets](/flyctl/cmd/fly_secrets) like so:
	```bash
	fly secrets set AWS_ACCESS_KEY_ID="<TIGRIS_ACCESS_KEY_ID>"
	fly secrets set AWS_SECRET_ACCESS_KEY="<TIGRIS_SECRET_ACCESS_KEY>"
	```

5. **[Upload a file](https://github.com/fly-apps/go-example-tigris/blob/main/tigris/tigris.go#L26):** Aside from connecting with a Bucket, we can use other [AWS SDK functions](https://docs.aws.amazon.com/code-library/latest/ug/go_2_s3_code_examples.html). Let's use the `PutObject` function to  upload a text file on the go like so:
