...
sitemap: false
redirect_from: /docs/getting-started/sveltekit/
---
...
In this example, to deploy your app on Fly.io, we'll install the Node Adapter. You can install it by executing the command `npm i -D @sveltejs/adapter-node` and then adding it to our `svelte.config.js` file. Here's all the code:
...
The `start` command is used to start your application, and you will need it before you deploy. Add the following line to your `package.json` file:
