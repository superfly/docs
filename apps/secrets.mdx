...
source_url: https://fly.io/docs/apps/secrets/
---
...
If you need secrets to be available when building your Docker image, see [Build secrets](/apps/build-secrets).
...
The `secrets` command can also take secrets from `stdin`. For commands and options, see the [`fly secrets` docs](/flyctl/cmd/fly_secrets) or run `fly secrets --help`.
...
You can use this with other `[[files]]` entries, check the `fly.toml` [reference](/reference/configuration#the-files-section) for more details.
