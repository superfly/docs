...
source_url: https://fly.io/docs/litefs/proxy/
---
...
[configuring writes to the primary node](/litefs/getting-started-docker#configuring-writes-to-primary-node)!
</Info>
...
(`POST`, `PUT`, etc) are forwarded to primary node using [`fly-replay` header](/networking/dynamic-request-routing).
...
[proxy section](/litefs/config#http-proxy-server) of the config:
...
which is `8081` in this example. Additionally, make sure your [internal port](/reference/configuration#the-services-sections)
