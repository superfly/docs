...
source_url: https://fly.io/docs/monitoring/logs-api-options/
---

  <img src="/images/logs-api-options.png" alt="Illustration by Annie Ruygt of a rocket ship, a satellite dish, a cargo container, and a clock, all representing different options for logs" />

## Overview

Fly.io apps run on Firecracker microVMs we call Machines. Each Machine captures `stdout`/`stderr`, ships those logs over [NATS](https://nats.io), and stores them for a period of time in a VictoriaLogs-backed search index. Most users consume logs via `fly logs` or by setting up log shipping to an external sink.
...
For more details on connecting to Fly’s NATS log stream (authentication, subject patterns, example clients), head over to the [Observability for User Apps](/blueprints/observability-for-user-apps#streaming-fly-app-logs-to-your-end-users) guide.
...
- [Fly Docs: Export logs](/monitoring/exporting-logs)
