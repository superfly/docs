...
source_url: https://fly.io/docs/monitoring/metrics/
---

  <img src="/images/monitor-machines.png" alt="Illustration by Annie Ruygt of some Machines strutting along" />
...
You’ll need to authenticate with a [Fly Access Token](/security/tokens). Depending on the token type, the header uses either the Bearer format (`Authorization: Bearer <token>`) or the FlyV1 format (`Authorization: FlyV1 <token>`). You may only query series scoped to your organizations.
...
If you don't already run Grafana or Prometheus, Fly.io's [Observability for User Apps](/blueprints/observability-for-user-apps) guide deploys a ready-made stack (VictoriaMetrics and Grafana) you can build alerts on.
...
- `app`: App name
- `region`: [Fly.io Region](/reference/regions#fly-io-regions)
...
The following CPU metrics are related to [CPU Performance](/machines/cpu-performance):
...
Labels:
- `mount`: mount point name(s), `/` and if using [Volumes](/volumes), the destination name in fly.toml.
...
If you're using [Volumes](/volumes) for any of your organization's apps, you'll be able to query these series,
...
If you have a [Postgres](/unmanaged-postgres) database hosted on Fly.io, you'll automatically get the following series,
...
If your app uses [multiple processes](/launch/processes), you can add multiple `[[metrics]]` sections, each with its own set of `processes`:
