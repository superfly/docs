---
title: Deploy an app
source_url: https://fly.io/docs/launch/deploy/
---

  <img src="/images/docs-apps.webp" alt="" />
...
Whether you've run [`fly launch`](/launch/create) with the `--no-deploy` option, or want to apply changes to your deployed app's source or configuration, you can make a new release by running 
...
If you haven't deployed the app before, `fly deploy` creates one or two Machines for every [process group](/launch/processes) defined in the app's `fly.toml` file, depending on whether there are services and volumes configured. If you haven't explicitly defined any process groups, the first deployment gives you two Machines for redundancy. For more information about when Fly Launch creates one Machine or two Machines, refer to [Redundancy by default on first deploy](/apps/app-availability#redundancy-by-default-on-first-deploy).
...
There are [a number of other options](/flyctl/cmd/fly_deploy) you can use with the `fly deploy` command.
...
2. Otherwise, check the [`[build]` section of `fly.toml`](/reference/configuration#the-build-section) and use the method specified there, whether it's a Dockerfile or a buildpack (but don't use buildpacks if you don't have to; they're brittle, bloated, and prone to change).
...
If the app to be deployed is configured with an eligible HTTP `[[services]]` section, and it does not yet have [public IP addresses](/networking/services), `fly deploy` provisions a public IPv6 and a shared public IPv4 Anycast address. 
...
If a Machine has a mounted [volume](/volumes), `fly deploy` can't be used to mount a different one. You can change the mount point at which the volume's data is available in the Machine's file system, though. This is configured in the [`[mounts]` section](/reference/configuration#the-mounts-section) of `fly.toml`.

Learn more about [using Fly Volumes](/volumes).

## `fly deploy` configuration in `fly.toml`

Configure the following deployment behavior in the [`[deploy]` section](/reference/configuration#the-deploy-section) of `fly.toml`.

### Release commands
You can run a one-off [release command](/reference/configuration#run-one-off-commands-before-releasing-a-deployment) in a temporary VM&mdash;using the successfully built release&mdash;before that release is deployed. This is good for, e.g., running database migrations. Keep in mind that these temporary VMs do not have access to any volumes you may have mounted in your apps.
...
Learn more about [setting a deployment strategy](/reference/configuration#picking-a-deployment-strategy).
...
You can [add an IP address](/networking/services#ip-addresses) to an App, for example, without redeploying.

Adding a [secret](/apps/secrets) to the App does require a restart, so `fly secrets set` triggers a new deployment.
...
- [Create a new app with Fly Launch](/launch/create)
- [Troubleshooting your deployment](/getting-started/troubleshooting)
