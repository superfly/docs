...
source_url: https://fly.io/docs/launch/scale-count/
---

  <img src="/images/docs-drones.webp" alt="" />
...
* [Automatically Stop and Start App Machines](/launch/autostop-autostart), for adjusting active capacity based on traffic.
* [Autoscale based on metrics](/launch/autoscale-by-metric), to use the autoscaler app to scale Machines based on any metric.
* [`fly machine` commands](/flyctl/cmd/fly_machine), including [`start`](/flyctl/cmd/fly_machine_start) and [`stop`](/flyctl/cmd/fly_machine_start) subcommands to target individual Machines with flyctl.
* Machines API [stop](/machines/api/machines-resource#stop-a-machine) and [start](/machines/api/machines-resource#start-a-machine) endpoints.
</Note>

There are two ways to change the number of Machines managed by [Fly Launch](/launch/index) after deploying an app for the first time:
...
When Machines are created or destroyed using `fly scale count` or `fly machine clone`/`fly machine destroy`, the resulting scale is preserved by `fly deploy`&mdash;except in the case that you scale right down to zero Machines. If there are no existing Machines, then `fly deploy` seeds the app with new Machines in the `primary_region` and according to the `[processes]` configured in your `fly.toml` file. For more information about how many Machines are created when you launch a new app or deploy from zero, refer to [Redundancy by default on first deploy](/apps/app-availability#redundancy-by-default-on-first-deploy).

## View the app's current scale {#view-the-apps-current-scale}

`fly scale show` outputs all the scale information about an app's Machines: how many Machines of each VM specification, in each of the app's process groups, in each [region](/reference/regions).
...
The following examples apply to a Fly App's default process group: the `app` process group. If you don't define [process groups](/launch/processes), then all of the app's Machines belong to the `app` process group, with the exception of any Machines created with `fly machine` commands or the Machines API.
...
## Scale an app's regions {#scale-an-apps-regions}

The `--region` option takes one or more [region codes](/reference/regions). `fly scale count` creates and/or destroys Machines to reach the specified target count across the regions you list in this option, and tries to balance its changes across these regions.
...
- [Scale Machine CPU and RAM](/launch/scale-machine) How to use vertical scaling.
- [Autoscale based on metrics](/launch/autoscale-by-metric) Learn about automatic scaling using metrics.
- [flyctl scale — CLI reference](/flyctl/cmd/fly_scale) Read a full breakdown of the scale subcommands. 
- [Machine Placement and Regional Capacity](/machines/guides-examples/machine-placement) Find out how region choice and resource availability affect scaling success.
