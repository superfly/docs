...
source_url: https://fly.io/docs/security/shared-responsibility/
---
...
There's also a lot of software involved, but the important ones are flyd which manages Fly Machines on a worker, [Fly Proxy](/reference/fly-proxy) which is the frontend proxy service that runs on edges, and flyctl which runs on your client devices.

## Fly.io's responsibilities {#fly-ios-responsibilities}
