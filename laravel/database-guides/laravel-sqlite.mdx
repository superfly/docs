...
og:description: Notes and configurations on using the bundle-able SQL database engine, [SQLite](https://www.sqlite.org/index.html)
source_url: https://fly.io/docs/laravel/database-guides/laravel-sqlite/
---
...
2.  As seen above, the SQLite database will be located in the storage directory. Make sure to mount a volume on the directory by following [this guide here](/laravel/the-basics/laravel-volume-storage).

    [Mounting a volume](/launch/volume-storage) on the storage folder will ensure everything it contains gets persisted, including data on the SQLite database located inside.
...
Do you have multiple instances set up for your Fly app? Data between those instances' SQLite databases will not automatically be in sync with each other. You'll have to configure data replication across those instances' SQLite databases by yourself. Of course, manual setup is not a requirement&mdash;you can instead rely on the *power* of [LiteFS](/litefs).

LiteFS provides data replication across different SQLite databases of your Fly app. All you’ll need is to set it up on your Fly app, and it will do the replication for you. You can read more about the [getting started guide here](/laravel/advanced-guides/global-sqlite-litefs), and also check out our Laravel section’s quick notes on setting LiteFS on a [Laravel Fly app here](/laravel/advanced-guides/global-sqlite-litefs).
