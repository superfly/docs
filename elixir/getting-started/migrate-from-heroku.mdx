...
The most important thing you'll want to be comfortable with using Fly.io's database offering is [backing up and restoring your database](/postgres/managing/backup-and-restore).

As your application grows, you'll probably first [scale disk and RAM resources](/postgres#scaling-vertically-adding-more-vm-resources), then [scale out with multiple replicas](/postgres#scaling-horizontally-adding-more-replicas). Common maintenance tasks will include [upgrading Postgres](/postgres#upgrading-the-postgres-app) as new versions are released with new features and security updates.

[See here](/postgres/getting-started/what-you-should-know) for a more comprehensive guide for what's required when running your Postgres databases on Fly.io.
