...
source_url: https://fly.io/docs/elixir/getting-started/migrate-from-heroku/
---
...
* [Connect via IEx](/elixir/the-basics/iex-into-running-app)
...
### Custom Domain & SSL Certificates {#custom-domain-amp-ssl-certificates}

After you finish deploying your application to Fly.io and have tested it extensively, [read through the Custom Domain docs](/networking/custom-domain) and point your domain at it.

In addition to supporting [`CNAME` DNS records](/networking/custom-domain#option-i-cname-records), Fly.io also supports [`A` and `AAAA` records](/networking/custom-domain#option-ii-a-and-aaaa-records) for those who want to point `example.com` (without the `www.example.com`) directly at their App on Fly.io.
...
Check out the [flyctl docs](/flyctl) for a more extensive inventory of flyctl commands.
...
To achieve the desired `git push` behavior, we recommend setting up `fly deploy` as the final command in your continuous integration pipeline, as outlined for GitHub in the [Continuous Deployment with Fly.io and GitHub Actions](/launch/continuous-deployment-with-github-actions) docs.
...
Fly.io can be configured to deploy on git commits with the following techniques with a [GitHub Action](/launch/continuous-deployment-with-github-actions).
...
#### Heroku's managed database {#herokus-managed-database}
...
The most important thing you'll want to be comfortable with using Fly.io's database offering is [backing up and restoring your database](/unmanaged-postgres/managing/backup-and-restore).

As your application grows, you'll probably first [scale disk and RAM resources](/unmanaged-postgres#scaling-vertically-adding-more-vm-resources), then [scale out with multiple replicas](/unmanaged-postgres#scaling-horizontally-adding-more-replicas). Common maintenance tasks will include [upgrading Postgres](/unmanaged-postgres#upgrading-the-postgres-app) as new versions are released with new features and security updates.

[See here](/unmanaged-postgres) for a more comprehensive guide for what's required when running your Postgres databases on Fly.io.

### Pricing

Heroku and Fly.io have very different pricing structures. You'll want to read through the details on [Fly.io's pricing page](/about/pricing) before launching to production. The sections below serve as a rough comparison between Heroku's and Fly.io's plans as of August 2022.
...
Fly.io pricing is [metered for the resources](/about/pricing) you use. Database is billed by the amount of RAM and disk space used, not by rows. The closest equivalent to the Heroku Hobby tier on Fly.io looks like this:
