...
og:description: Cluster your erlang nodes into a single distributed network!
source_url: https://fly.io/docs/elixir/the-basics/clustering/
---
...
Fly.io makes it super easy to run VMs of your applications physically closer to your users. Through the magic of DNS, users are directed to the nearest [region](/reference/regions) where your application is located.
...
- [Put it as a secret](/apps/secrets#set-secrets) inside project settings under the `RELEASE_COOKIE` name, or
...
After setting up the ENV and deploying the application, we can verify that the cookie is being used by getting an [IEx shell into our running server](/elixir/the-basics/iex-into-running-app) and issuing the following command:
...
An empty list means the node has no connections. If you are sure that there is more then one node running, you could proceed to [Troubleshooting](/elixir/the-basics/troubleshooting) documentation.
