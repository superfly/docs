...
og:description: Guide for connecting a locally running Livebook to your application running on Fly.
source_url: https://fly.io/docs/elixir/advanced-guides/connect-livebook-to-your-app/
---
...
![WireGuard livebook connection](/images/elixir-livebook-to-server-fly-overview.webp?2/3&centered)
...
- **Known cookie value on your server**. Livebook needs to know the cookie value used on the server. Follow [this guide to give your app a static cookie value](/elixir/the-basics/clustering#the-cookie-situation).
- **WireGuard setup on your local machine**. Follow the Fly.io [Private Network VPN](/networking/private-networking#private-network-vpn) guide to walk through that.
...
![Runtime settings location on sidebar](/images/livebook-data-analysis-runtime-click-step1.webp?centered&card)
...
![Runtime settings configure connection](/images/livebook-data-analysis-runtime-click-step2.webp?centered&card)

Now we use "Attached Node" to connect to an existing Elixir node.

![Runtime settings configure connection](/images/livebook-data-analysis-runtime-click-step3.webp?centered&card)

With the "Attached Node" mode selected, we need to provide 2 additional pieces of information before we can connect.

![Attach steps](/images/livebook-runtime-attach-steps.webp?centered&card)
...
We also need the cookie used by the server. See [this guide](/elixir/the-basics/clustering) for setting that up for your app.
...
Looking for things to do now that you're connected? Check out: [Do Interesting Things with Livebook and Your Production App](/elixir/advanced-guides/interesting-things-with-livebook)
