...
source_url: https://fly.io/docs/app-guides/minio/
---
...
We're going to set up a single instance of MinIO on Fly.io, backed by [Fly Volume storage](/volumes).
...
This is a guide to a demonstration deployment of the official standalone MinIO Docker image, and isn't tuned in any way. Check out the [MinIO docs](https://min.io/docs/minio/linux/index.html) docs for more sophisticated use.
...
There's no need for this app to be accessible from the public internet, if you run your object storage within the same [Fly IPV6 private network](/networking/private-networking) as the app(s) that want to connect to it.
...
One way to do this is to set up a [regular WireGuard tunnel](/networking/private-networking), and visit `http://my-minio.internal:9001` in the browser.
...
One of the great things about Fly.io is the [private networking](/networking/private-networking) we get for free. We'll use our Machines' private IPs to define hostnames in `/etc/hosts` which our nodes will use to communicate with each other.
