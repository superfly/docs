...
source_url: https://fly.io/docs/app-guides/mysql-on-fly/
---

If you just want to run a quick, self-managed MySQL instance on Fly.io, here's how to do it. It's pretty basic, with one caveat around using a [Fly Volume](/volumes) to persist data.
...
To connect to your MySQL database from outside of your Fly organization, you need a WireGuard connection. However, `fly` on your local machine can connect using [user-mode WireGuard](https://fly.io/blog/our-user-mode-wireguard-year/) magic, without you having to set up your own WireGuard tunnel.

You can forward the MySQL server port to your local machine using [`fly proxy`](/flyctl/cmd/fly_proxy):
