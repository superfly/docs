...
source_url: https://fly.io/docs/blueprints/custom-deploy-workflows/
---

  <img src="/images/custom-deploy-workflows.png" alt="Illustration by Annie Ruygt of a Frankie the hot air balloon writing on a notepad about different modes of transport including a jet a car and a truck" />
...
- [Machines overview](/machines): Learn how Machines work and what makes them different.
- [fly deploy CLI reference](/flyctl/cmd/fly_deploy): Full list of deploy options including `--only-machines`, `--regions`, and `--process-groups`.
- [Dynamic request routing](/networking/dynamic-request-routing): Use `fly-replay` to shift traffic between machines on the fly.
