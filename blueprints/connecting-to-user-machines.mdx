...
source_url: https://fly.io/docs/blueprints/connecting-to-user-machines/
---

  <img src="/images/connecting-to-users.png" alt="Illustration by Annie Ruygt of a bird holding hands with a computer" />
...
![HTTP Request Flow](/images/blueprints/connecting-to-user-machines-http-flow.svg)
...
3. Sends a `fly-replay` response header with parameters like `app` and `instance` to route the request to the correct app and machine. For more details on the parameters, see the [fly-replay documentation](/networking/dynamic-request-routing#the-fly-replay-response-header).
...
3. On the user machine, parse the `fly-replay-src` header. This header contains fields such as `instance`, `region`, `t`, and `state` (see [fly-replay documentation](/networking/dynamic-request-routing#the-fly-replay-response-header)).
...
- [Per‑User Dev Environments with Fly Machines](/blueprints/per-user-dev-environments) Find out how to run isolated, on‑demand environments for each user.
- [Observability for User Apps](/blueprints/observability-for-user-apps) Learn about how to gather logs and telemetry from per‑user apps/machines once you’ve routed traffic to them.
- [Dynamic Request Routing with fly‑replay](/networking/dynamic-request-routing) Learn about the `fly‑replay` header and dynamic request routing.
