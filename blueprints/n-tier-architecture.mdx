...
source_url: https://fly.io/docs/blueprints/n-tier-architecture/
---

  <img src="/images/get-started-n-tier.png" alt="Illustration by Annie Ruygt of a bird slicing a loaf of bread-think app layers" />
...
- **Web tier**: the app servers/backend that [handle requests](/networking/dynamic-request-routing) and run business logic
...
- **Data tier** → [Fly Managed Postgres](/postgres) (our hosted Postgres that handles the messy parts for you)
...
- **Let the database do its job**: With [Managed Postgres](/postgres), you don’t need to worry about setup, backups, or failover.
...
Now create your **data tier** with [Fly Managed Postgres](/postgres):
...
More details on creating and connecting a managed Postgres database can be found [here](/postgres/create-and-connect).
...
[Fly’s proxy](/reference/fly-proxy) takes care of spreading requests across the machines (load balancing).
...
- **Data tier**: [Fly Managed Postgres](/postgres)
...
- Explore the features of [Fly Managed Postgres](/postgres) 
- Find out how to [scale apps on Fly](/launch/scale-count)
- Read about [Fly networking](/networking) 
