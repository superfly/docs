...
source_url: https://fly.io/docs/blueprints/opensshd/
---

  <img src="/images/openssh.png" alt="Illustration by Annie Ruygt of a terminal window, a computer and some numbers" />
...
One way to use these tools is to [set up a wireguard VPN](/networking/private-networking#install-your-wireguard-app) and
[issue a new SSH credential](/flyctl/cmd/fly_ssh_issue).  This may be impractical for some use cases (example: github actions).
...
Before proceeding, a **caution**: unless you are **certain** that all of the clients will access this service through IPv6, you will need a [dedicated IPv4](/networking/services#dedicated-ipv4) address.
...
There are a [number of ways to run multiple processes](/app-guides/multiple-processes).  The most straightforward
...
The following assumes that you have a [volume](/launch/volume-storage) mounted at `/volume`, and an entrypoint
script.
...
- [`flyctl` SSH commands](/flyctl/cmd/fly_ssh) Official reference for how to use `fly ssh`/`fly ssh sftp` to access running machines. 
- [Multiple processes inside a Fly.io app](/app-guides/multiple-processes) Explains how to run multiple process groups in a single machine/container. Relevant when you’re adding `sshd` alongside your main app.
