...
source_url: https://fly.io/docs/blueprints/resilient-apps-multiple-machines/
---

  <img src="/images/resilient-apps.png" alt="Illustration by Annie Ruygt of an armoured truck with a Fly balloon logo" />
...
You pay only for `rootfs` when Machines aren't running, not for CPU and RAM. Machine rootfs is [cheap](/about/pricing#stopped-fly-machines), like 18 cents a month for an average Elixir app that uses 1.2 GB of rootfs.
...
#### 1. Set up autostop/autostart {#1-set-up-autostop-autostart}

Use [Fly Proxy autostop/autostart](/launch/autostop-autostart) to automatically stop and start Machines based on traffic. Keep one or more Machines running in your primary region if you want to. Example `fly.toml` config:
...
Fly Proxy uses the concurrency `soft_limit` to determine if Machines have excess capacity. Learn more about [how Fly Proxy autostop/autostart works](/reference/fly-proxy-autostop-autostart).

**Using the Machines API:** To add or change the autostop/autostart settings with the Machines API, use the settings in the `services` object of the [Machine config](/machines/api/machines-resource#machine-config-object-properties) in your `create` or `update` calls.
...
Learn more about [scaling the number of Machines](/launch/scale-count).

**Using the Machines API:** To scale the number of Machines with the Machines API, you can "clone" Machines. Get and modify a Machine config and use that config to create new Machines in any region. See the[ Machines resource docs](/machines/api/machines-resource).
...
Unlike Fly Proxy autostop/autostart, which starts Machines based on app traffic, a standby Machine watches the Machine it's paired to and starts only if that Machine becomes unavailable. Learn more about [standby Machines](/apps/app-availability#standby-machines-for-process-groups-without-services).
...
**Using the Machines API:** To create a standby Machine with the Machines API, get and modify your worker Machine config and use that config to create a new standby Machine. Specify the standby in the Machine's config object: [config.standbys](/machines/api/machines-resource#machine-config-object-properties).
...
- [App availability and resiliency](/apps/app-availability): An overview of the all features that can make your app more resistant to events like hardware failures or outages.
- [Multi-region with Fly-Replay](/blueprints/multi-region-fly-replay): How to serve reads close to users and route writes back to a primary region.
- [Machine placement and regional capacity](/machines/guides-examples/machine-placement): How Fly decides where to place your Machines, how to guide placement yourself, and what happens when a region runs out of capacity.
- [Run Ordinary Rails Apps Globally](https://fly.io/ruby-dispatch/run-ordinary-rails-apps-globally/): Project-level example of Rails distributed across regions with smart routing.
