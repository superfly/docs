...
source_url: https://fly.io/docs/blueprints/going-to-production-with-healthcare-apps/
---

  <img src="/images/production-healthcare.png" alt="Illustration by Annie Ruygt of a balloon doctor using stethescope on an app" />
...
- **At Rest**: Fly.io ensures data at rest is secured on [encrypted NVMe user volumes](/volumes), aligning with the encryption standards required by HIPAA for storage security.
...
- **Secrets Management**: Fly.io provides a [secrets manager](/apps/secrets) for storing and handling API credentials and environment variables securely, ensuring that sensitive information is accessible only to authorized applications.
...
- **Data Storage and Backup**: Fly.io’s infrastructure provides robust data integrity controls, including [regular snapshots](/volumes/volume-manage) of [encrypted volumes](/volumes/overview#volume-encryption) to prevent data loss.
...
- **Regional Data Control**: Fly.io allows users to control the [physical region](/reference/regions) where data is stored and processed using simple commands, facilitating compliance with data residency requirements of HIPAA by ensuring PHI is stored within designated secure regions, such as the United States.

There's more detail at [https://fly.io/docs/about/healthcare](/about/healthcare) and [https://fly.io/security](https://fly.io/security). If you have additional questions about Fly.io's security infrastructure & practices, please [reach out](mailto:sales@fly.io).
...
If your free demo account doesn't have enough resources for your pilot application, you may [upgrade to a paid plan](/about/pricing) or [contact us](mailto:sales@fly.io) to make other arrangements.
...
Fly.io's [secret manager stores](/apps/secrets) your application's API credentials and secrets in a secure vault. The contents remain encrypted and are only available from running Machine instances as environment variables.
...
- [Healthcare apps on Fly.io](/about/healthcare) Overview of how Fly.io supports HIPAA‑compliant apps, including encryption, network isolation, and audit‑controls.
- [Going to production checklist](/apps/going-to-production) A general production‑readiness checklist for apps on Fly.io.
