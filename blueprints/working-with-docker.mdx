...
source_url: https://fly.io/docs/blueprints/working-with-docker/
---

  <img src="/images/Docker-With-Fly.png" alt="Illustration by Annie Ruygt of a Frankie the hot air balloon giving flowers to the Docker whale" />
...
Fly now also supports multi-container Machines. If you're looking for more information and use cases, check out [this forum post](https://community.fly.io/t/using-containers-with-flyctl/24729) and [this guide](/machines/guides-examples/multi-container-machines).
...
The filesystem is ephemeral. If your app writes files, installs packages, or modifies anything at runtime, those changes vanish when the machine stops. On the next boot, it’s back to the clean image. Want to keep data around? Mount a [volume](/volumes/overview).
...
That’s expected. Fly.io Machines reset their root filesystem each time they start. Use [volumes](/volumes/overview) for persistence.
...
- **Avoid hardcoded secrets.** For _runtime_ [secrets](/apps/secrets), use `fly secrets`. It’s built for exactly this, and injects secrets securely into your app’s environment. For _build-time_ secrets (like private package tokens), use [build secrets](/apps/build-secrets) to avoid leaking them into your final image.
...
- [**Deploy with a Dockerfile**](/languages-and-frameworks/dockerfile) Find out how `fly launch` and `fly deploy` work when you're starting from a Dockerfile.
- [**Managing Docker images with the Fly.io registry**](/blueprints/using-the-fly-docker-registry) Understand how image storage, tagging, and reuse work in Fly.io’s private registry.
- [**Using base images for faster deploys**](/blueprints/using-base-images-for-faster-deployments) Learn how shared base images can shave time off your CI/CD loop.
...
- [**Customizing your Dockerfile for Laravel**](/laravel/advanced-guides/customizing-dockerfile) A Laravel-specific deep dive, but useful even if you’re not using Laravel.
