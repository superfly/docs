...
source_url: https://fly.io/docs/blueprints/long-running-tasks/
---
...
If you're picking a queue technology or a cron runner, start with the [work queues](/blueprints/work-queues) or [task scheduling](/blueprints/task-scheduling) guides instead. This page is about the machine behavior underneath them.
...
**Multiple machines.** The proxy uses your `soft_limit` [concurrency setting](/blueprints/setting-concurrency-limits) to compute excess capacity:
...
- A few log lines may be lost across a suspend/resume cycle, and the system clock can take a second or two to re-synchronize after resume. See "[Suspend vs. Stop](/getting-started/troubleshooting#suspend-vs-stop)" for details on clock skew.
...
Tradeoff: you're paying for at least one worker machine continuously. If your work is batchy enough that on-demand workers make sense, use the [work queues guide's on-demand worker pattern](/blueprints/work-queues) instead, as that spins up a fresh machine per job and lets it stop when done.
...
| Cron-style scheduled jobs | See [task scheduling](/blueprints/task-scheduling) |
| Queue-driven workers | B: combine with [work queues](/blueprints/work-queues) |
| One-off jobs (fire and forget per request) | On-demand workers; see [work queues](/blueprints/work-queues) |
...
- [Work queues guide](/blueprints/work-queues): Picking a queue technology
- [Task scheduling guide](/blueprints/task-scheduling): cron-style triggers and scheduled machines
- [Autostart and autostop reference](/reference/fly-proxy-autostop-autostart): The proxy's full decision logic
- [Configuration reference](/reference/configuration): `kill_signal`, `kill_timeout`, `processes`, `auto_stop_machines`
- [Machine states](/machines/machine-states):  what `stopping`, `stopped`, and `suspended` actually mean
