...
og:description: How to create staging environments on the Fly.io with GitHub actions.
source_url: https://fly.io/docs/django/advanced-guides/staging-environments-with-github-actions/
---
...
- `jobs → preview-app → if`: skips deploy on PRs without the
  <span className="bg-indigo-500 rounded text-white px-1 py-1">_PR preview app_</span>
...
- `jobs → preview-app → steps → with`: specifies all inputs that we want to pass to our
  ...
  [region](/reference/regions#fly-io-regions) and organization as a
  starting point:
...
If you don't have a [Postgres cluster](/unmanaged-postgres) specifically for
testing purposes you can create one with [`fly postgres create`](/flyctl/cmd/fly_postgres_create):
...
use a custom [Fly.io TOML configuration file](/reference/configuration)
dedicated for staging with a release script
[to run before a deployment](/reference/configuration#run-one-off-commands-before-releasing-a-deployment).
