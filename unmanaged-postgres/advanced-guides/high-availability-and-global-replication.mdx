...
og:description: Set up a Postgres cluster, configure replicas, perform failovers, and learn what tradeoffs
  should be considered for a global Postgres deployment.
source_url: https://fly.io/docs/postgres/advanced-guides/high-availability-and-global-replication/
noindex: true
---
...
4. Open the `fly.toml` file and set the [`primary_region`](/reference/configuration#primary-region) and the `PRIMARY_REGION` environment variable to your new target region. For example, to move leadership into the iad region:
