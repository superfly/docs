...
og:description: '`fly pg attach`: Conveniently create a new database in a Fly Postgres cluster for
  a Fly app, and give the app credentials to connect.`fly pg detach`: Conveniently
  revoke an app''s access to a Fly Postgres database.'
source_url: https://fly.io/docs/postgres/managing/attach-detach/
---
...
<PostgresWarning />
  ...
  <img src="/images/attach-detach.webp" alt="" />
...
Run `fly postgres users list --app <postgres app name>` on your Postgres cluster app. If there is an attached Fly App, then the app's name will be in the list of users. See [Users & Roles](/unmanaged-postgres/managing/users-and-roles).
