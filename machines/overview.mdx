...
source_url: https://fly.io/docs/machines/overview/
---

  <img src="/images/fly-machines-intro.png" alt="Illustration by Annie Ruygt of some birds launching a Fly rocket" />

Fly Machines are fast-launching VMs with a simple [REST API](/machines/api). They're [the compute](https://fly.io/blog/fly-machines/) behind the Fly.io platform. If you've launched an app on Fly.io, you're already using Fly Machines.

We use the [Machines API](/machines/api) to build the orchestration for [Fly Launch](/apps). You can use it however you'd like.
...
out to more regions and more Machine counts. [Fly Launch](/apps) does all that for you with simple syntax and configuration.
...
that is why you'd use Fly Machines directly, rather than [letting flyctl run your deployment](/launch/deploy). 

To get your head around that trick, start by understanding the lifecycle of a Fly Machine:

1. You create a Fly Machine with a [Create Machine request](/machines/api/machines-resource#create-a-machine), or 
...
2. Once the Fly Machine exists, we boot it up for you right away. The Machine is in `started` state. It's running. You can talk to it. This happens fast! Everything's already assembled; we're just booting. Usually this takes _well under a second_. The same goes for starting a stopped Machine with a [Start Machine request](/machines/api/machines-resource#start-a-machine), or with `fly machine start`.
   
3. You're done with the Fly Machine, so you stop it with a [Stop Machine request](/machines/api/machines-resource#stop-a-machine), or `fly machine stop`. The VM shuts down. The Machine is in `stopped` state. Its components are still assembled on our worker host, ready to start back up; if you want to do that, `GOTO 2`.
   
4. You're tired of the Fly Machine, and want it to go away permanently. Send a [Delete Machine request](/machines/api/machines-resource#delete-a-machine-permanently), or use `fly machine destroy`. We clear the resources we were holding for the Machine off our server. You can easily create and start a new Machine from the same image, but it'll be slower than stopping and starting an existing Machine.

Learn more about [running a new Machine](/machines/flyctl/fly-machine-run) or [updating a Machine](/machines/flyctl/fly-machine-update) with flyctl commands.

### Scaling Machines

<Note>
Remember, the ordinary way to scale an application on Fly.io is to use [Fly Launch](/apps), which offers convenient
...
<Info>
**App-scoped static egress IPs are recommended.** The machine-scoped static egress IPs described below are a legacy feature. For most use cases, you should allocate [app-scoped static egress IPs](/networking/egress-ips#static-egress-ips-app-scoped) instead. Unlike machine-scoped IPs, app-scoped IPs can be shared between multiple machines in a region and are not deleted when machines are recreated.
</Info>

A machine-scoped static egress IP is attached to a single machine and isn't shared with other machines. It survives a [machine migration](/reference/machine-migration).
...
If supported, it is recommended to use [WireGuard](/networking/private-networking#private-network-vpn) to connect to external services.
...
Read more about Machine placement and regional capacity in [this guide](/machines/guides-examples/machine-placement).
...
- [Machine states and lifecycle](/machines/machine-states) Find out how Machines move through states (created → started → stopped etc), what it means, how to detect issues.
- [Machine placement and regional capacity](/machines/guides-examples/machine-placement) Learn more about how region and host capacity affect where your Machines land and what to do if placement fails.
- [`fly machine` CLI reference](/flyctl/cmd/fly_machine) Use this command‑line reference for managing Machines via `flyctl` (create, update, stop, clone, etc).
- [Working with the Machines API](/machines/api/working-with-machines-api) Read a practical guide to creating, updating, and managing Machines via API calls.
- [Machines API reference](/api/machines) Use this low-level reference for the Machines API runtime, generated from the OpenAPI specification.
