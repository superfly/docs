...
source_url: https://fly.io/docs/machines/guides-examples/machines-app-using-flyctl/
---

<Warning>
# This Document is Stale!

This document illustrates some ways to work with Fly Machines individually using flyctl, and was written before Fly Apps V2 made it possible to manage Machines as a group, using a `fly.toml` config file and `fly deploy`. For most web apps, the [Fly Apps docs](/apps) are a better place to start!
</Warning>

We talk up the Machines API for running user code, for ephemeral jobs, or both. Fly Machines are also the VM building blocks for V2 of the [Fly.io app-platform-as-a-service offering](/launch/index).
...
The [`fly machine` command docs](/flyctl/cmd/fly_machine) are another good resource.
...
1. A Machines-flavoured [Fly App](/apps/overview)&mdash;you're not using `fly deploy` to manage the app's Machines, but you still need an app, as it's the administrative entity that owns your VMs and things like anycast IPs and volumes
...
Note the allocation of a [shared IPv4 address](/networking/services#shared-ipv4) above. For regular HTTP(S) web apps, shared IPv4 works fine, and helps conserve the world's dwindling IPv4 supply (and can [save you money](/about/pricing#anycast-ip-addresses)). If your app runs other services and you need a dedicated IPv4, you can provision one by leaving out the `--shared` flag.
...
While there's no need to go down to the level of controlling individual Machines for a simple web app&mdash;you'd just run `fly launch` to get a [Fly App](/apps) configured and have the conveniences of `fly deploy`&mdash;we can do it if we want to.  
...
![Browser screenshot saying 'This page isn't working; testrun.fly.dev didn't send any data. ERR_EMPTY_RESPONSE'](/images/testrun-err-empty-response.webp)
...
If I were working with a [Fly App](/apps/overview) instead of with Machines directly, I'd edit the internal port in my `fly.toml` and redeploy. I'll do the Fly Machines equivalent: `fly m update` with the modification I want to make to the configuration.
