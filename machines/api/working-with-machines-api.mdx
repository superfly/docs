...
og:description: Get started with the Fly Machines API.
source_url: https://fly.io/docs/machines/api/working-with-machines-api/
---
...
**[Apps resource](/machines/api/apps-resource)** - Every Fly Machine belongs to a [Fly App](/apps/overview).

**[Machines resource](/machines/api/machines-resource)** - Fly Machines are our fast-launching VMs.

**[Tokens resource](/machines/api/tokens-resource)** - Support for OpenID Connect tokens.

**[Volumes resource](/machines/api/volumes-resource)** - [Fly Volumes](/volumes) are persistent storage for Machines.
...
From within your Fly.io [private WireGuard network](/networking/private-networking), you can connect to the API directly using the internal endpoint. From outside the Fly.io WireGuard mesh, use the public endpoint; this proxies your request to the API.
...
To set an access token as an environment variable on Fly Machines, use a [secret](/apps/secrets); for example:
...
| 422 | Validation failed (for example, an app name that is already taken). |
...
This applies to all actions except [Get Machine](/machines/api/machines-resource#get-a-machine) which is 5 req/s, with a short-term burst limit up to 10 req/s.
