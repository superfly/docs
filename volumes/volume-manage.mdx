...
source_url: https://fly.io/docs/volumes/volume-manage/
---

import VolumesAccessSnippet from '/snippets/docs/volumes-access.mdx';

<Info>
**For Fly Launch apps:** Learn how to [create a new app with volumes or add a volume to your app](/launch/volume-storage).
</Info>

You can manage Fly Volumes using the [`fly volumes`](/flyctl/cmd/fly_volumes) command.

Fly Volumes are local persistent storage for [Fly Machines](/machines). Learn [how Fly Volumes work](/volumes/overview).
...
Use the `--region` option to set a [region](/reference/regions), or select a region when prompted.
...
For more options, refer to the [`fly volumes create` docs](/flyctl/cmd/fly_volumes_create) or run `fly volumes create --help`.
...
You can also configure a volume to extend automatically after a certain threshold [using the Machines API](/machines/api/volumes-resource#extend-a-volume) or [in the [mounts] section](/reference/configuration#auto-extend-volume-size-configuration) of your app's `fly.toml` file.
...
For options, refer to the [`fly volumes extend` docs](/flyctl/cmd/fly_volumes_extend) or run `fly volumes extend --help`.
...
3. (Optional) Use [`fly scale count`](/launch/scale-count#scale-an-app-with-volumes) to create a new Machine that picks up the unattached volume, or attach the volume to a new Machine by cloning with the following command:

```bash
`fly machine clone <machine id> --region <region code> --attach-volume <volume id>:<destination mount path>`
```

For options, refer to the [`fly volumes fork` docs](/flyctl/cmd/fly_volumes_fork) or run `fly volumes fork --help`.

A detailed guide to [volume forking](/blueprints/volume-forking) is also available.
...
Clone a Machine with a volume to create a new Machine in the same region with an empty volume. Use the `-r` option to clone the Machine into a different [region](/reference/regions).
...
For options, refer to the [`fly machine clone` docs](/flyctl/cmd/fly_machine_clone) or run `fly machine clone --help`.
...
For options, refer to the [`fly volumes destroy` docs](/flyctl/cmd/fly_volumes_destroy) or run `fly volumes destroy --help`.
...
You can restore data from a deleted volume using a volume snapshot. At this time, you can only retrieve a deleted volume's ID for 24 hours after deletion. If you already have the volume ID, then you can still use it to access the volume's snapshots within the snapshot retention period. The default snapshot retention period is 5 days. Learn how to [set or change the snapshot retention period](/volumes/snapshots#set-or-change-the-snapshot-retention-period).
