...
source_url: https://fly.io/docs/volumes/overview/
---

  <img src="/images/volumes.png" alt="Illustration by Annie Ruygt of a filing cabinet" />
...
* **A volume belongs to one app**: Every Fly Volume belongs to a [Fly App](/apps/overview) and you can't share a volume between apps.
...
Explore other options for data storage in [Databases & Storage](/database-storage-guides).
...
You'll also need to replicate data between volumes. Some options for replicating databases include [LiteFS - Distributed SQLite](/litefs) and [Supabase Postgres](/supabase).
...
You can create volumes at the same time you create Machines, or ahead of time. If you try to create new Machines through [`flyctl scale count`](/launch/scale-count#scale-an-app-with-volumes), then `flyctl` will first check if you have any unattached volumes in the region you are creating new Machines in. If it finds unattached volumes, it will try to create the new Machines on the same servers. If it does not find any unattached volumes, then it will create new ones together with the Machines. You can also create unattached volumes through [`fly volumes create`](/flyctl/cmd/fly_volumes_create).

Volumes exist on a single server in a single [region](/reference/regions). For redundancy within a region, you can run multiple Machines with attached volumes. Remember that volumes don't automatically replicate data between them.

To prevent a single server hardware failure from taking down your app, it is better if each volume is placed in a separate hardware zone. A separate hardware zone is just another way of saying a different server. We default to separate hardware zones when you create volumes with the [`fly volumes create`](/flyctl/cmd/fly_volumes_create) command, but this is not guaranteed when creating Fly Volumes through other methods. Note that having each volume in a separate hardware zone limits the number of volumes your app can have in a region. If you need more volumes in a region than there are distinct hardware zones, you can set `--require-unique-zone` to `false` when you run [`fly volumes create`](/flyctl/cmd/fly_volumes_create).

You can hint what type of compute load you expect to use the volume for by using the `--vm-XXX` options with the [`fly volumes create`](/flyctl/cmd/fly_volumes_create) command. For example, if you plan to attach a `performance-4x` Machine to the volume, set `--vm-size=performance-4x` to place the volume on a host that has capacity for that Machine. Without a hint, your volume can end up on a host with room for the volume but not for the Machine you want to attach to it.
...
The default volume size is 1GB when you create a volume with the `fly volumes create` command, and when you use the `fly launch` command to [launch an app with a volume](/launch/volume-storage#launch-a-new-app-with-a-fly-volume). The maximum volume size is 500GB.

You can extend a volume's size&mdash;either [manually](/volumes/volume-manage#extend-a-volume) or [automatically](/reference/configuration#the-mounts-section)&mdash;to make it larger, but you can't shrink a volume.
...
You can also [create a snapshot of a volume](/volumes/snapshots#create-a-volume-snapshot) on demand.

You can [restore a volume snapshot](/volumes/snapshots#restore-a-volume-from-a-snapshot) into a new volume of equal or greater size.

You can also [disable automatic daily snapshots](/volumes/snapshots#disable-automatic-daily-snapshots).
...
* **Release Command**: Volumes are also not mounted to the temporary Machine created when using a [release_command](/reference/configuration#run-one-off-commands-before-releasing-a-deployment).
