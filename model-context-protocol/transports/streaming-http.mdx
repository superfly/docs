...
og:description: Streaming HTTP servers can be deployed as is.
source_url: https://fly.io/docs/mcp/transports/streaming-http/
---
...
With Streaming HTTP transports, [`fly mcp proxy`](/flyctl/cmd/fly_mcp_proxy) may not be required, but could be useful if:
* Your MCP client doesn't support Streaming HTTP
* You need to set up a [wireguard tunnel](/flyctl/cmd/fly_proxy) to access your MCP serever via an [`.internal`](/networking/private-networking) or [`.proxy`](/networking/flycast) address.
