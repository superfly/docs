...
og:description: Shows how to use a wireguard tunnel and flycast to access a proxy
source_url: https://fly.io/docs/mcp/access-control/flycast/
---
...
Every Fly Organization has a [private network](/networking/private-networking). In most cases, you will want **only** a private v6 address on applications that are not available on the internet.
...
With this in place you can use [`fly proxy`](/flyctl/cmd/fly_proxy) to create a tunnel, or you can follow our blueprint to [Jack into your private network with WireGuard](/blueprints/connect-private-network-wireguard).
 ...
 *  `.flycast` addresses target an _external_ port for your application, and supports [fly routing headers](/networking/dynamic-request-routing#alternative-routing-headers). If your request is routed to a machine that is stopped or suspended, that machine will be started first. Again the protocol to use is `http` not `https`, so an typical URL would look like `http://mcp.flycast/`.

 [Flycast - Private Fly Proxy services](/networking/flycast) provides more information on the use of Flycast.
