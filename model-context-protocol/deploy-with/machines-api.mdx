---
title: Machines API
og:description: This guide shows you how to use the Machines API to create a Fly.io machine that runs
  an MCP server remotely.
source_url: https://fly.io/docs/mcp/deploy-with/machines-api/
---

The [Machines API](/machines/api) is a low level interface that provides access to the full range of platform services.
...
An example of when you would want to use the Machines API is when you want to create a [Per-User Dev Environment](/blueprints/per-user-dev-environments)
...
![Obtaining an organization token via the dashboard](/images/orgtoken.png)
...
The host name you chose depends on whether your application is running [inside or outside your Fly.io private Wireguard network](/machines/api/working-with-machines-api#api-addresses). If you are not sure, use the public base URL:
...
Now use the [Create a Fly App](/machines/api/apps-resource#create-a-fly-app) API:
...
This next part contains a lot of [properties](/machines/api/machines-resource#machine-config-object-properties), so first an overview:
...
Normally you would access the MCP using [`fly mcp proxy`](/flyctl/cmd/fly_mcp_proxy) to create a STDIO MCP server for your MCP client, even if you are using the Machines API to deploy an existing MCP server on a Fly Machine.

But for those interested in lower level details, [`fly mcp wrap`](/flyctl/cmd/fly_mcp_wrap) supports the asynchronous nature of MCP servers by requiring all requests be done via HTTP POST and all replies are returned in response to a HTTP GET request.
...
* There are [rate limits](/machines/api/working-with-machines-api#rate-limits) in place.
