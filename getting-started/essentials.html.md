...
Learn more about [Fly Launch](/docs/apps/index-launch/).
...
[**Fly Apps**](/docs/apps/overview): The way Machines are grouped for admin and management on the Fly.io platform.

[**Fly GPUs**](/docs/gpus/): Machines, but with GPUs. They boot up with GPU drivers installed and you can run `nvidia-smi` right away.

[**Fly Launch**](/docs/apps/index-launch/): Our orchestrator that includes some good stuff for app hosting, like the `fly launch` command to get started, `fly.toml` for configuration, the `fly deploy` command to deploy all your app's Machines into new versions/releases, and the `fly scale` command to scale Machines.
