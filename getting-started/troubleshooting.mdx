...
source_url: https://fly.io/docs/getting-started/troubleshooting/
---

  <img src="/images/troubleshooting.png" alt="Illustration by Annie Ruygt of a figure looking through a magnifying glass at a balloon" />
...
- **Review fly.toml:** Run `fly config validate` to catch syntax and configuration errors. Double-check formatting, [port numbers](#your-app-isnt-listening-on-the-right-address), and recent changes against the [configuration reference](/reference/configuration).
...
If all Machines are stopped and you expect auto-start to wake them, verify your `[[services]]` or `[http_service]` config — auto-start only works when the proxy knows where to route traffic. See [Autostart and autostop](/launch/autostop-autostart).
...
- Check your [Cloudflare](/networking/understanding-cloudflare) proxy settings
...
### Build hangs: Waiting for depot builder... {#build-hangs-waiting-for-depot-builder}
...
[Release commands](/reference/configuration#deploy) run in an ephemeral Machine before your app starts.
...
Secrets set with `fly secrets set` are available as environment variables at runtime. They're not available at build time. If you need build-time values, use `[build.args]` in `fly.toml`. Find out more about build-time secrets [here](/apps/build-secrets).
...
- Move large assets to a [volume](/volumes) or object storage
...
## Your app isn't listening on the right address {#your-app-isnt-listening-on-the-right-address}
...
Health checks tell the Fly proxy whether your Machine is ready to receive traffic. If a Machine fails its health checks, the proxy stops routing requests to it. If all your Machines fail health checks, your users get 503s. For the full picture on how health checks work, see [Health checks](/reference/health-checks).
...
For CPU-intensive apps, make sure you've selected an appropriate [Machine size](/about/pricing#started-fly-machines). CPU and RAM scale together in preset combinations.
...
**Using Cloudflare?** Most TLS issues on Fly involve domains behind Cloudflare. Read [Understanding Cloudflare](/networking/understanding-cloudflare) before debugging further. For all other setups, see [Custom domains](/networking/custom-domain).

### Flycast (internal load balancing)

[Flycast](/networking/flycast) routes traffic between your Fly apps over the private network. Two gotchas:
...
MPG clusters run on Fly's private network and aren't accessible over the public internet. Connection strings use `.flympg.net` domains, which resolve to private network addresses. See [Create and connect to MPG](/postgres/create-and-connect) for full details.
...
- WireGuard: connect to your org's private network, then use the `.flympg.net` connection string directly. Read more in this [reference guide](/blueprints/connect-private-network-wireguard).
...
If you don't have snapshots, the data may be unrecoverable. **Always enable snapshots for volumes with data you care about.** See [Volume snapshots](/volumes/snapshots).

---

## Dashboard and account access

### Can't log in (GitHub SSO) {#cant-log-in-github-sso}
...
Fly runs on bare metal in [17 regions](/reference/regions). Individual hosts or regions can have issues independent of the rest.
...
- [App configuration reference](/reference/configuration)
- [Fly Machines](/machines)
- [Networking on Fly.io](/networking)
- [Autostart and autostop](/launch/autostop-autostart)
