...
MPG clusters run on Fly's private network and aren't accessible over the public internet. Connection strings use `.flympg.net` domains, which resolve to private network addresses. See [Create and connect to MPG](/mpg/create-and-connect) for full details.
